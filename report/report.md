...
### Эварист Галуа 
> {слайд 5 - Эварист Галуа, на слайде картинка Галуа, еще можно мем с Drake, где Маск|Tony Stark и Галуа}
...
> {слайды 6, 7 - свойства полей и арифметика по модулю}

Перейдем к сущности понятия `конечные поля`. Если совсем по-простому, то поле - это просто ограниченный набор чисел, например: {0, 1, 2, 3, 4}. Но есть нюанс. Когда мы производим математические операции над элементами поля, результат должен остаться внутри поля. Но если мы сложим элементы поля $P_{5}$ 3 и 4 то получим число 7, а оно не лежит в $P_{5}$. Непорядок. Получается нужно переопределить операции над элементами так, чтобы результат оставался в поле. Эти все операции определяются на основе арифметики по модулю. Итого получается 3 + 4 = 2. Вообще для определения конечного поля, нужно чтобы кроме элементов поля были определены операции сложения и умножения, которые удовлетворяют следующим условиям:
...
> {слайд 8 - зачем нужны вычисления в полях}
...
> {слайд 9 - график `эллиптической кривой`}
...
> {слайды 10, 11, 12 - иллюстрация операции сложения, пояснения про свойства операции сложения}
...
> {слайд 13 - вид графика прямой над конечным полем, вид графика эллиптической кривой над конечным полем}
...
> {слайд 14 - нужно отобразить нелинейный характер операции}

Мы уже подбираемся непосредственно к криптографии. Пока не ясно как всю эту математическую дичь можно использовать для того, чтобы при соединении с банком никто не увел ваш пароль от личного кабинета, хотя график `эллиптической кривой` на `конечном поле` наводит на мысль, что какая-то случайность в этом всем есть, а значит за ней можно как-то спрятать свой трафик. На всякий случай, если вы до сих пор видите, только график и точки, то нам нужно перейти в область цифр. График - это всего лишь визуальное отображение реальных цифр. Например, возьмем `уравнение эллиптической` кривой вида \(y^2=x^3+7 \) в `конечном поле` состоящем из цифр от 0 до 103, обозначается так: \(F_{103} \). Точка на плоскости обозначается двумя цифрами: значением по осям \(X \) и \(Y \). Выберем {x=17, y=64}. Давайте убедимся, что эта точка лежит на нашей кривой:
...
\end{align*}
```
Если посмотреть на цифры, то какой-то закономерности не просматривается. А теперь представьте, что у вас есть значение исходной точки (47, 71) и точки полученной на каком-нибудь из этапов. Как найти множитель? Эта задача называется дискретным логарифмированием и выполняется простым перебором всех вариантов, в нашем случае 20. Таким образом сложность вычисления равна \(O(n) \). Спойлер - если у вас получится перебрать все варианты, то вы сможете взломать TLS. Как будто криптографическая стойкость не очень скажете вы, но давайте посмотрим что будет дальше.

## Криптография на эллиптических кривых

### Определение криптографической кривой

> {слайд 15 - характеристики `криптографической кривой` биткоина и стойкость шифрования}

Для осознания стойкости криптографии на `эллиптических кривых` давайте для примера возьмем `эллиптическую кривую` используемую для шифрования в bitcoin. Она называется *secp256k1*. Для этого быстренько вспомним что мы узнали про `эллиптические кривые` в `конечных полях`:
* общий вид уравнения `эллиптической кривой` имеет вид \(y^2=x^3+ax+b \), то есть для работы с конкретной кривой нужно знать значения параметров `a` и `b`;
* порядок конечного поля, над которым заданы значения `эллиптической кривой`, или количество элементов в этом поле;
* нужна исходная генераторная точка G в формате координат x и y;
* количество элементов в группе, которую можно породить из исходной точки G, умножая ее на 1, 2, 3, ... и так далее, пока не доберемся до бесконечно удаленной точки, то есть нулевого элемента поля.
Задав, все эти значения говорят, что задана `криптографическая кривая`. Таким образом при handshake сторона, которая инициирует соединение, говорит приемной стороне: "для шифрования используем `криптографическую кривую` *secp256k1*", при этом приемная сторона понимает какие параметры нужно использовать при соединении.
Для `криптографической кривой` биткоина эти значения следующие:
...
Что мы здесь видим? Количество значений n, которые нужно перебрать, чтобы взломать шифрование близко к \(2^{256} \). Но много это или мало? \(2^{256} ~ 10^{77}\) и немного цифр для сравнения:
...
> {слайд 16 - алгоритмы подписания и верификации}

И так. Посмотрели что там под капотом, узнали про кучу всякой математической фигни, узнали про то, какое офигенно криптографически стойкое шифрование на `эллиптических кривых`. Но до сих пор особо ничего не понятно. Как вся эта лабуда связана с теми вещами, которыми мы оперируем в реальной жизни, когда сталкиваемся с криптографией.

Давайте еще раз повнимательнее взглянем на выражения скалярного умножения: (\eG = P\), здесь (\e\) - это просто числа от 1 до (\n\), которое количество элементов в группе, (\G\) - исходная точка группы, а (\P\) - просто какое-то число, результат умножения. Мы помни, что найти (\e\) можно только перебором, то есть почти невозможно. Получается (\P\) - открытый ключ, (\e\) - это секретный ключ. Вот и все.

Вы скажете: "какая-то белиберда". Секретный и открытый ключи - просто два случайных больших числа, как тогда с помощью открытого ключа зашифровать сообщение так, чтобы его можно было расшифровать с помощью закрытого. Ответ - никак, такое есть в алгоритме RSA. А что делают здесь? Основная фишка криптографии на `эллиптических кривых` - это подписание и верификация. 
